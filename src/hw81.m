...
for j=1:Ny
    for i=1:Nx
        r1=sqrt((XX(i,j)-(-5))^2+(YY(i,j)-0)^2);
        r2=sqrt((XX(i,j)-(-1))^2+(YY(i,j)-0)^2);
        r3=sqrt((XX(i,j)-(5))^2+(YY(i,j)-0)^2);
        r4=sqrt((XX(i,j)-(1))^2+(YY(i,j)-0)^2);
        if r1<rc1
...
dt=10^-5;
while t<T 
    [ux,uy,w]=multisolver(X,Y,w,ux,uy,Nx,Ny,dx,dy,dt, cycle, XX, YY);
